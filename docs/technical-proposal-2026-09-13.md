...
从解析 AST 提取 H1–H6，保持层级与文档顺序，支持左侧固定栏、窄屏抽屉、折叠和当前节高亮。不需要扫描文件系统，也不要求文件夹授权。

### 4.2 文件夹目录树

用户从扩展的顶层工作区页面点击“选择文件夹”，调用 `showDirectoryPicker({ mode: 'read' })`。只对选中范围建立文件树；没有授权时正常保留单文件阅读，不把整机目录作为默认数据源。[S2]
...
PR 执行类型检查、单测、安全语料与 Chromium 扩展 E2E；生产构建完成后检查 manifest、CSP、外部可执行资源、源文件泄露和第三方声明。构建动作与运行依赖固定版本，公开工作流使用受审查的 action 提交。
